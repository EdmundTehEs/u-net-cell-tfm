...
from utils.ViTNet import ViTNet
...
calculated_input_channels = len(protein.split(",")) # Assuming protein is 'zyxin' or 'zyxin,actin'

    model_kwargs={
                    'input_type':  protein, 
                    'input_channels': calculated_input_channels, # Dynamically set
                    'output_channels': out_channels[0], # Use the first element of out_channels tuple (e.g., 2 for (Fx, Fy))
                    'img_size': transform_kwargs['crop_size'],
                    'patch_size': 16,
                    'embed_dim': 768,
                    'depth': 12,
                    'num_heads': 12,
    ...
    model = ViTNet( **model_kwargs, model_idx=1, device=device)
    # model.to(device) # This line becomes redundant if device is passed to __init__ and handled there
    ...
    if save_models:
        ...
        if e%(logger_params["save_model_freq"])==0 or e==n_epochs-1: 
            checkpoint = {
                ...
                "model_kwargs": model_kwargs, # Save model_kwargs to reconstruct the model
                "dataset_kwargs": dataset_kwargs,
                "test_cells": dataset.test_cells,
                # Add any other relevant information like loss history, etc.
            }
            torch.save(checkpoint, os.path.join( model.logdir, "model_checkpoint.pt")) # Use a more descriptive name
...
eval_dataset_kwargs['transform_kwargs']['crop_size'] = 512
